...
The differentiator is **Llavero**, an AI agent built on Codex that:
...
| AI | Vercel AI SDK **v6** + AI Gateway → `anthropic/Codex-sonnet-4-6` (fallback: direct via `@ai-sdk/anthropic` + `ANTHROPIC_API_KEY`) | Mock fallback `LLAVE_OFFLINE=1` for offline demos |
...
|----------|----------|---------|
| `AI_GATEWAY_API_KEY` | preferred | Routes `'anthropic/Codex-sonnet-4-6'` via Vercel AI Gateway |
