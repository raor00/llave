...
export default async function OnboardingPage({
  ...
  // UNLESS they explicitly entered with ?edit=1 to update their profile.
  if (!isEdit && profile?.role && profile?.full_name && ROLE_HOME[profile.role]) {
    ...
    <div className="container-x py-16 max-w-3xl">
      <div className="text-center mb-10">
        <span className="chip mb-3">
          {isEdit ? "Mi Llave · Editar perfil" : "Mi Llave · Onboarding"}
        </span>
        <h1 className="font-display text-4xl md:text-5xl font-bold leading-tight">
          {isEdit ? "Actualiza tu perfil" : "¿Cómo quieres usar Llave?"}
        </h1>
        <p className="mt-4 text-lg text-[color:var(--color-fg-muted)] max-w-xl mx-auto">
          {isEdit
            ? "Cambia tu rol, datos o contraseña. Al guardar te llevamos al dashboard correspondiente."
      ...
      <OnboardingForm
        ...
        defaultRole={(profile?.role as "inquilino" | "asesor" | "propietario" | null) ?? null}
        email={user.email ?? ""}
        editMode={isEdit}
      />
    </div>
